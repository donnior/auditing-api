CREATE MATERIALIZED VIEW report_summary_mv AS
SELECT
    r.id,
    r.employee_id,
    r.eval_period,
...
CREATE UNIQUE INDEX ux_report_summary_mv_id
    ON report_summary_mv(id);

CREATE INDEX idx_report_summary_mv_sort
    ON report_summary_mv(eval_period DESC, eval_type, employee_id);

CREATE INDEX idx_report_summary_mv_employee_period_type
    ON report_summary_mv(employee_id, eval_period DESC, eval_type);

CREATE INDEX IF NOT EXISTS idx_xca_evaluation_detail_summary_key
    ON xca_evaluation_detail(employee_id, eval_period, eval_type);
